...
import { apiBankAccountsGet, apiBankAccountsDelete } from 'api/bankAccounts';
import { reducer, initialState } from 'state/bankAccount.reducer';
import { ADD_FILTER, UPDATE_FILTER, DELETE_FILTER, CLEAR_FILTERS } from 'state/filter.types';
import { DELETE, ERROR_FETCH, CLEAR_ERRORS, READ_ALL, CLEAR_ITEMS } from 'state/bankAccount.types';
...
class BankAccountTableContainer extends Component {
  constructor(props) {
    super(props);
  ...
  async fetchData() {
    ...
    if (authenticated) {
      try {
        ...
        const { bankAccounts, headers } = await apiBankAccountsGet(serviceUrl, requestParameters);
        const bankAccountCount = (headers && headers['X-Total-Count']) || 0;

        this.dispatch({
          type: READ_ALL,
          payload: {
            items: paginationMode === 'infinite-scroll' ? [...items, ...bankAccounts] : bankAccounts,
  ...
  async handleDelete(item) {
    ...
    if (authenticated) {
      try {
        await apiBankAccountsDelete(serviceUrl, item.id);
        onDelete(item);
        ...
        <AuthenticatedView keycloak={keycloak}>
          ...
          <PaginationWrapper items={items} paginationMode={paginationMode} count={count}>
            <div className={classes.tableWrapper}>
              <BankAccountTable items={items} onSelect={onSelect} Actions={Actions} />
            </div>
          </PaginationWrapper>
        </AuthenticatedView>
        <Notification
...
BankAccountTableContainer.propTypes = {
...
BankAccountTableContainer.defaultProps = {
  onAdd: () => {},
  onDelete: () => {},
  onError: () => {},
  onSelect: () => {},
...
export default withKeycloak(
  withStyles(styles)(
    withTranslation(undefined, { withRef: true })(withPagination(BankAccountTableContainer))
  )
);
