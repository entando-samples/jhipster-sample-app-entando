...
1. Go to widget source (<MICROSERVICE_FOLDER>/ui/widgets/label/formWidget/>):
   1. Run `npm i`
...
- **label.form.createError** (fired when an error occurs during label creation)
- **label.form.create** (fired when the label is successfully created)
- **label.form.updateError** (fired when an error occurs during label update)
- **label.form.update** (fired when the label is successfully updated)
